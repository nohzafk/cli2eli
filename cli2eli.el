...
TOOL is the current JSON."
  (let* ((tool-name (alist-get 'tool tool))
         (command-vector (alist-get 'commands tool))
         (commands (append command-vector nil)))
    (dolist (cmd commands)
...
If RELATIVE-P is non-nil, treat JSON-FILE as relative to the package directory."
  ...
  (let* ((file-path (expand-file-name json-file))
    ...
    (cli2eli--remove-replaced-functions cli2eli--current-tool)
